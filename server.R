...
function(input, output, session) {
  ...
  observeEvent(input$Btn_GetFile, {
    ...
    if (nrow(file_selected)) {
      ...
      updateSelectInput(session, "my_res", choices = returns$bin_width.lst, selected = returns$bin_width.lst[1])
      #validate(need(input$my_res != "", message = "resolution loading..."))
      ...
      updateSelectInput(session, "my_chr", choices = returns$chr.df$names, selected = returns$chr.df$names[1])
  ...
  observeEvent(list(returns$mcool.path, input$my_balanced, input$my_chr, input$my_res, input$balanced_name), {
    ...
    validate(
      need(returns$chr_name != "", message = "loading mcool metadatas..."),
      need(returns$bin_width != "", message = "loading mcool metadatas..."))
  ...
  observeEvent(list(input$my_log2, returns$from, returns$to, returns$matrix), {
    ...
    validate(need(!is.null(returns$subset_matrix), message = "loading matrix..."))
  ...
  output$render_MATplot <- renderPlot({
    validate(need(!is.null(returns$MATplot), message = "start by uploading a mcool file"))
    returns$MATplot}) 
  ##################################
  ##################################
  ##################################
